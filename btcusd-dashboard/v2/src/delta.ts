...
export async function getAllPositions(apiKey: string, apiSecret: string): Promise<DeltaPositionResponse> {
  return deltaRequest<DeltaPositionResponse>({
    method: 'GET',
    path: '/v2/positions?underlying_asset_symbol=BTC',
    apiKey,
    apiSecret,
    label: 'getAllPositions',
    retries: 2,
  });
}
...
export async function getProducts(apiKey: string, apiSecret: string) {
  return deltaRequest({
    method: 'GET',
    path: '/v2/products',
    apiKey,
...
export async function getTickers(apiKey: string, apiSecret: string, symbol?: string) {
  let path = '/v2/tickers';
